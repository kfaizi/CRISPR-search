...
# Modified by Kian Faizi to send output on success (GFF results can be found..,
#   CRISPRS found.., seq#..) to STDOUT, rather than STDERR (June 2019).
#   Now only failures are captured in error output. Also made Finder consistent
#   "v2"
...
my $NumberofPossibleCrisprs = 0;
while ( $seq = $seqIO->next_seq ) {
    $inputfileCount++;
    $inputfile = $basename;
    #print STDERR "inputfile is $inputfile\n";
    ...
    print STDOUT "seq$inputfileCount... ";
    ...
    my @vmatchoptions =
      qw(-l 23 25 60 -e 1 -s leftseq -evalue 1 -absolute -nodist -noevalue -noscore -noidentity -sort ia -best 1000000 -selfun sel392.so 55);
    ...
    if ( $#rep >= 0 ) {
        ...
        $totalNumberOfCrisprs += $nbrcris;    #LK/PDH
        $NumberofPossibleCrisprs += $OneSpacerCris_nbr; #PDH
    }
    ...
    # cwd is ./out/gcf
    #print STDERR "<<<< topa: ", cwd, " >>>>\n";
    chdir "..";                                         #LK - ./out
    #print STDERR "<<<< topb: ", cwd, " >>>>\n";
    unlink <..\/crispr_result_*>;                       # remove stuff in ./
    unlink <..\/seq_v*>;                                # ./
    unlink <..\/$inputfile.*>;
    unlink <..\/spacersseq_v*>;
    unlink '../stdout';
    ...
    unlink '../DR';                                     # ./
    unlink <*_CRISPRs>;                                 # ./out
    unlink "../$inputfile";                             # ./gcf.fna
    #print STDERR "inputfile is $inputfile\n";
    #print STDERR "<<<< topc: ", cwd, " >>>>\n";
    chdir "..";
    #print STDERR "<<<< top1: ", cwd, " >>>>\n"; # ./ then ./out
    `rm -Rf $outdir-$inputfileCount &>/dev/null`; # remove file five-n

    #print STDERR "$ResultDir, $outdir, $inputfileCount\n"; # out, five, n


    `mv $ResultDir/$outdir $ResultDir/$outdir-$inputfileCount`;  #LK - move out/five to out/five-n

    #print STDERR "<<<< top2: ", cwd, " >>>>\n"; # ./ then ./out
...
my $seqIdPrefix =
  ( split( /\//, $userfile ) )[-2];    # prefix is just before assembly.fasta
my $fastaFilename = makeFasta( $gffFilename, $userfile, $seqIdPrefix );
print STDOUT "CRISPRFinder found $totalNumberOfCrisprs CRISPRs\n";
print STDOUT "CRISPRFinder found $NumberofPossibleCrisprs possible CRISPRs\n-------\n";
...
sub makeFasta {
    ...
    my $seqout =
      Bio::SeqIO->new( -format => "fasta", -file => ">$fastaFilename" );
    my $crisprCounter = 0;
    while (<GFF>) {
        ...
        my (
            $seqid, $source, $type,  $start, $stop,
            $phase, $strand, $score, $attributes
        ) = split(/\t/);
        ...
        my $desc =
            "Length="
          . ( $stop - $start + 1 )
          . " Start=$start Stop=$stop Contig="
          . $seq{$seqid}->id . "";
        my $seqObj =
          Bio::Seq->new( -id => $id, -seq => $subseq, -desc => $desc );
...
sub makeGff {
    ...
    # parse
    my $GFFstr = "";
    open GFF, ">$ResultDir/$inputBasename.gff"
      or die
"Could not open output GFF in $ResultDir/ because $!\nI will not create the GFF file.\n";
    for my $dir (@dir) {
        ...
        for my $spacerFile (@spacerFile) {
            ...
            if ( $spacerFile =~ /_(\d+)$/ ) {
                ...
                warn
"Warning: Cannot understand result file $spacerFile. Skipping.\n";
                next;
            }

            # transform the report file to a GFF line
            my $reportFile =
              "$dir/$inputBasename" . "_PossibleCrispr_$spacerNumber";
            $reportFile = "$dir/$inputBasename" . "_Crispr_$spacerNumber"
              unless -e $reportFile;
            unless ( -e $reportFile ) {
    ...
    print GFF $GFFstr;
    close GFF;
    print STDOUT "GFF results can be found in $ResultDir/$inputBasename.gff\n";
...
sub reportToGff {
    ...
    open( REPORT, "<", $reportFile )
      or die("Could not open the report file $reportFile because $!\n");
    while (<REPORT>) {
...
# Construct the gff line with 1) the entire CRISPR and then 2)DRs interspersed with spacers
    ...
    for my $s (@spacerInfo) {
        ...
        $gffLine .= join( "\t",
            $seqid, $source, "CRISPRdr", $DRstart,
            ( $DRstart + $tag{DR_length} - 1 ),
            $score, $strand, $phase, "Parent=$featureId" )
          . "\n";
...
{ #function that returns a structure of hypothetical crisprs definition (the structure spacers )
    ...
    my ( @lines, $count, @temp, $posdeb, $posend, $len, $i, $refFalsSpacers,
        @spacers_correct );
    my @spacers   = ();
    my $nbspacers = 0;
    ...
    open( FD, $crisprfile )
      or die "Error in opening the input file : $crisprfile!";
    ...
    while ( my $line = <FD> ) {
        ...
        if ( $len < ( $DRlength * 0.6 ) ) {
            $mism =
              $temp[ $#temp - 1 ] eq "." ? 0 : $temp[ $#temp - 1 ] / $DRlength;
        }
        else {
    ...
    if ( $#spacers >= 0 ) {
        ...
        ( $refFalsSpacers, @spacers_correct ) =
          check_cris_div( $DRlength, $indexname, @spacers );
    }
    else {
        my @false_spacers = ();
        $refFalsSpacers = \@false_spacers;
        ...
        if ( ( $spLen > $comp ) ) {
            if ( $i == $#spacers ) {
                if ( $#spacers > 0 ) {
                    ...
                    my @lines = <FD>;
                    close(FD);
                    open WRITER, "> TmpSpacers"
                      or die "The file TmpSpacers cannot be edited !\n";
                    my $k = 0;
                    while ( $k < $#lines - 1 ) {
                    ...
                    ( $reffal_spacers, @spacers_new ) =
                      check_cris_div( $DRlength, $indexname, @spacers_new );
                }
            }
            else {
                if ( !$begtest ) {
                    ...
                    @spacers_new =
                      add_spacer( $spPos, $spLen, $j, @spacers_new );
                    $j++;
                    $begtest = 0;
                }
                else {
...
sub checkspacersAlign {
    ...
    eval {
        ...
        my @params =
          ( 'ktuple' => 2, 'matrix' => 'BLOSUM', 'QUIET' => 'QUIET' );
...
sub checkspacers {
    ...
    my @vmatchoptions = (
        "-l", $len, "-e $err", "-nodist", "-noevalue", "-noscore", "-noidentity"
    );
...
# the output is files containig each hypothetical crispr loci
    ...
    # print STDERR "seqencesdefinition is @seqencesdefinition\n"; # 33971 34030, length 60
    ...
    for ( $count = 0 ; $count < $#seqencesdefinition ; $count += 2 ) {
     ...
     #positions cases
        if ( $seqencesdefinition[$count] - 500 > 0 ) {
            # if we have more than 500 nt on the left, shift left bound -500
            # print STDERR "1\n";
            $seqencesdefinition[$count] = $seqencesdefinition[$count] - 500;
        }
        else {
            # print STDERR "2\n";
            $seqencesdefinition[$count] = 1;
        } # or set to position 1

        if ( $seqencesdefinition[ $count + 1 ] + 500 > $seq->length() ) {
            # if we have less than 500 on the right, set right bound to last nt
            # print STDERR "3\n";
            ...
            # print STDERR "4\n";
            $seqencesdefinition[ $count + 1 ] =
              $seqencesdefinition[ $count + 1 ] + 500; # or add 500
        }
        ...
        @subselectoptions = (
            "-range",
            $seqencesdefinition[$count] -1, # adh
            $seqencesdefinition[ $count + 1 ] -1, # adh
            $inputfile, ">", $seqfile
        );

        # print STDERR "for: subselectoptions2 are @subselectoptions\n";
        # print STDERR "for: seqencesdefinition is @seqencesdefinition\n";
...
sub write_clusters {
    ...
    my ( $count, $seqbeg, $seqend, $i, $j, $DR, $DRocc, $nbrcris, $DRlength, $inputfile_wc );
    $i       = 0;
    $j       = 0;
    $nbrcris = 0;
    ...
    for ( $count = 1 ; $count <= $#tabseq / 2 + 1 ; $count++ ) {
        ...
        my @DR_cand_occ =
          ();  # store the possible DRs after comparing the number of occurences
        ...
        while ( ( $i <= $#rep ) && ( $rep[$i]->Pos1 <= $seqend ) ) {
            ...
            if ( !check_DR( $DR, @DR_cand ) ) {
                ...
                $DRocc = DR_occ_rev( $DR, @rep )
                  ;    #number of occurences (rev comp included)
        ...
        while ( $c <= $#DR_cand_occ ) {
            ...
            my @fuzznucoptions = (
                "-sequence", $indexname, "-pattern", $DR_cand_occ[$c],
                "-pmismatch", $err, "-outfile", $crisprfile
            );
            ...
            if ( $#DR_cand_occ > 0 ) {
                ...
                ( $score, $TotDRerr ) = compute_Crispr_score( $crisprfile,
                    length( $DR_cand_occ[$c] ) );
                if ( $score <= $bestscore ) {
                    $bestscore     = $score;
                    $bestindex     = $c;
                ...
                ( $bestscore, $TotDRerr_best ) =
                  compute_Crispr_score( $crisprfile,
                    length( $DR_cand_occ[0] ) );
            }
            $c++;
        }    # boucle while
        ...
        my ( $crisOK, $simDRs, $CrisprBeg, $CrisprEnd, $RefSpacersH,
            $nbspacers, $RefFalsSpacers )
          = Find_theCrispr( $indexname, $DR, $count, $seqbeg, $seqend,
            $crisprfile );
        if ($crisOK) {
        ...
        if ($crisOK) {
            my $Crispr_file = fill_in_crisprfile(
                $simDRs,                $RefSeq,
                $ResultDir,             $nbrcris,
                $CrisprBeg,             $CrisprEnd,
                $DR,                    $nbspacers,
                "spacers" . $indexname, $RefFalsSpacers,
                %$RefSpacersH
            );
            ...
            if ( $modf != -1 ) {
                ...
                while ( $modf >= 0 ) {
                    if ( $crisprs_nbr - $hyp_cris_nbr >= 1 ) {
                        my $s = 1;
                        while ( $s <= $crisprs_nbr ) {
                            $inputfile_wc = "$RefSeq" . "_Crispr_" . $s;
                            if ( -e $inputfile_wc ) {
                                ( $s, $crisprs_nbr, $OneSpacerCris_nbr ) =
                                  modify_files( $inputfile_wc, $ResultDir,
                ...
                #print STDERR "modified_files ran, now in ", cwd, "\n";
                chdir "../.."; # back up to root
                #print STDERR "backing up ../.. to", cwd, "\n";
    ...
    #print STDERR "<<<< cwd at end of write_clusters: ", cwd, " >>>>\n";
    return ( $nbrcris, $OneSpacerCris_nbr );
}

#------------------------------------------------------------------------------
sub modify_files {
    my ( $inputfile, $ResultDir, $RefSeq, $rank, $crisprs_nbr, $HYPCris_nbr,
        $modf )
      = @_;
    my ( @lines, @temp, $nb_spacers, @nb_div, $newCrisNbr, @divi );

    #print STDERR "<<<< modify_files1: ", cwd, " >>>>\n"; # already in out/five
    my $dir = $RefSeq . "/";
    #print STDERR "$dir\n"; # five/

    # chdir($dir);

    # print STDERR "<<<< modify_files2: ", cwd, " >>>>\n";
    ...
    if ( $lines[$j] =~ '#' ) { }
        ...
        my (
            $file_new,  $file_old,  $l1,        $l2,         $id,
            $CrisprBeg, $CrisprEnd, $nbspacers, $Spfile_old, $Spfile_new
        );
        $file_old   = $inputfile;
        ...
        if ( $nbsp1 >= 1 ) {
            if ( $nbsp1 >= 3 ) { $file_new = "$inputfile" . "_test_" . $id; }
                ...
                $file_new = "$inputfile" . "_test_possible_" . $id;
                $HYPCris_nbr++;
            }
            $crisprs_nbr = create_file(
                $file_new, $file_old,  $l1,          $l2,
                $id,       $CrisprBeg, $CrisprEnd,   $nbsp1,
                $modf,     1,          $crisprs_nbr, @divi
            );
            create_spFile( $dir, $ResultDir, $RefSeq, $Spfile_new, $Spfile_old,
                0, $nbsp1 );
        }
        else { $crisprs_nbr--; }
        ...
        # get the wrong spacer length. need to be in $dir already, e.g. out/five
        my $sp = getSp( $file_old, $l1 );
        #print STDERR "wrong spacer length $sp";
        ...
        if ( $nbsp2 >= 1 ) {
            ...
            $crisprs_nbr = create_file(
                $file_new, $file_old,  $l1,          $l2,
                $id,       $CrisprBeg, $CrisprEnd,   $nbsp2,
                $modf,     2,          $crisprs_nbr, @divi
            );
            create_spFile( $dir, $ResultDir, $RefSeq, $Spfile_new, $Spfile_old,
                $sp_prev, $nbsp2 );
        }
        ...
        # print STDERR "<<<< modify_files3: ", cwd, " >>>>\n"; # ./out/five
        # print STDERR "dir is $dir\n"; # $dir is h02/
        # print STDERR "nbsp1 is $nbsp1, nbsp2 is $nbsp2\n"; # 6, 8

        # renaming the files
        ## NEED TO BE IN $dir already, e.g. out/five
        ...
        if ( $nbsp1 >= 1 && $nbsp2 >= 1 ) {
           ...
           # print STDERR "first if \n";
            ...
            for ( my $k = $crisprs_nbr - 1 ; $k >= $rank ; $k-- ) {
                # print STDERR "first if > for \n";
                $l = $k + 1;
                if ( -e "$RefSeq" . "_Crispr_" . $k ) { # if exists h02/h02_Crispr_n
                    # print STDERR "first if > for > if \n";
                    rename(
                        "$RefSeq" . "_Crispr_" . $k,
                        "$RefSeq" . "_Crispr_" . $l
                    );

                    # print STDERR "spacers rename 1 \n";
                    rename( "Spacers_" . $k, "Spacers_" . $l );
                }
                else {

                    # print STDERR "first if > for > else \n";

                    rename(
                        "$RefSeq" . "_PossibleCrispr_" . $k,
                        "$RefSeq" . "_PossibleCrispr_" . $l
                    );
                }
            }
        }
        else {
            # print STDERR "first else \n";
            for ( my $k = $crisprs_nbr - 1 ; $k >= $rank ; $k-- ) {
                $l = $k - 1;
        ...
        if ( $nbsp2 >= 1 ) {
            # print STDERR "second if \n";
            #print STDERR "$RefSeq, $inputfile, $rank \n"; # h02, h02_Crispr_1, 2
            ...
            if ( -e "$inputfile" . "_test_" . $rank ) {
                # print STDERR "second if > if \n";
                rename(
                    "$inputfile" . "_test_" . $rank,
                    "$RefSeq" . "_Crispr_" . $l
                ) or print "Error renaming second if > if!\n";
            }
            else {
                # print STDERR "second if > else \n";
                rename(
                    "$inputfile" . "_test_possible_" . $rank,
                    "$RefSeq" . "_PossibleCrispr_" . $l
                ) or print "Error renaming second if > else!\n";
            }

            # ___spacers files ----------
            # print STDERR "spacers rename 3 \n";
            #print STDERR "<<<< ", cwd, " >>>>\n"; # ./

            $Spfile_new = "Spacers_test_" . $rank;
            my $Spfile = "Spacers_" . $l;

            # print STDERR "Spfile_new is $Spfile_new\n";
            # print STDERR "Spfile is $Spfile\n";
        ...
        if ( $nbsp1 >= 1 ) {
            ...
            # print STDERR "third if \n";
            # print STDERR "$RefSeq, $inputfile, $l \n"; # h02, h02_Crispr_1, 1
            unlink( "$RefSeq" . "_Crispr_" . $l );
            if ( -e "$inputfile" . "_test_" . $l ) {
                # print STDERR "third if > if \n";
                rename(
                    "$inputfile" . "_test_" . $l,
                    "$RefSeq" . "_Crispr_" . $l
                ) or print "Error renaming third if > if!\n";
            }
            else {
                # print STDERR "third if > else \n";
                rename(
                    "$inputfile" . "_test_possible_" . $l,
                    "$RefSeq" . "_PossibleCrispr_" . $l
                ) or print "Error renaming third if > else!\n";
            }

            # ___spacers files ----------
            # print STDERR "spacers rename 4 \n";
...
sub create_file {
    my (
        $file_new, $file_old,  $l1,          $l2,
        $id,       $CrisprBeg, $CrisprEnd,   $nbspacers,
        $modf,     $ordre,     $crisprs_nbr, @divi
    ) = @_;
    my $File_Content = '';
    my @temp;
    ...
    while (<FD>) {
        ...
        if ( $. == 15 ) {
            ...
            $File_Content .=
"# Crispr_begin_position: $CrisprBeg\t Crispr_end_position: $CrisprEnd";
        }
...
#####################################
sub create_spFile {
    my ( $dir, $ResultDir, $RefSeq, $Spfile_new, $Spfile_old, $sp_prev, $nbsp )
      = @_;
    $sp_prev = $sp_prev * 2 + 1;
    $nbsp    = $sp_prev + $nbsp * 2 - 1;
    #print STDERR "create_spFile dir is $dir, ResultDir is $ResultDir\n";
    #print STDERR "\n<<<< create_spFile cwd1: ", cwd, " >>>>\n";    # ./out/h02
    my $File_Content = "";
    open( FD, "$Spfile_old" ) # adh
      or die "Error in opening the file:$Spfile_old!";
    ...
    close(FD);
    open WRITER, "> $Spfile_new" # adh
      or die "The file $Spfile_new cannot be edited !\n";
    print WRITER $File_Content;
    close(WRITER);
...
sub getSp {
    ...
    # print STDERR "\n<<<< getSp cwd1: ", cwd, " >>>>\n";    # ./out
    # print STDERR "file_old is $file_old, l1 is $l1\n";
...
sub modifySumUpfile {
    ...
    my $tmp             = "tmp";
    my $File_Content    = "";
    open( FD, "$dir/$inputrefseqfile" )
      or die "Error in opening the file:$inputrefseqfile!";
    while (<FD>) {
        if ( $. == 10 ) {
            $File_Content .= "# Nbr_of_good_CRISPRs : $newCrisNbr\n";
        }
        elsif ( $. == 11 ) {
            $File_Content .= "# Nbr_of_one_Spacer_CRISPRs : $HYPCris_nbr\n";
        }
        else { $File_Content .= $_; }
    }
    close(FD);
...
sub create_recap {
    ...
    unless ( -d $directory ) {
        mkdir $directory
          or die "$0: I can not create the folder $directory: $!\n";
    }
    my $dir = $directory . "/" . $RefSeq;

    unless ( -d $dir ) {
        mkdir $dir or die "$0: I can not create the folder $dir: $!\n";
    }
    ...
    $File_Content .=
      "# Number of questionable structures : $OneSpacerCris_nbr\n";
    $File_Content .= "# \n";
    $File_Content .= "#=======================================\n";
    chdir($dir);
    open WRITER, "> $Crispr_report"
      or die "The file $Crispr_report cannot be edited !\n";
    print WRITER $File_Content;
    close WRITER;
}
...
sub fill_in_crisprfile {
    ...
    my (
        $simDRs,      $RefSeq,         $ResultDir, $id,
        $CrisprBeg,   $CrisprEnd,      $DRseq,     $nbspacers,
        $SpacersFile, $RefFalsSpacers, %spacersPos
    ) = @_;
...
"#=========================================================================\n";
    ...
    $File_Content .=
      "# Crispr_begin_position: $CrisprBeg\t Crispr_end_position: $CrisprEnd\n";
    $File_Content .=
      "# DR: $DRseq\t DR_length: $DRlength\t Number_of_spacers: $nbspacers\n";
    $File_Content .=
"#=========================================================================\n";
    $File_Content .=
      "Spacer_begin_position\t Spacer_length\t Spacer_sequence\n";
    ...
    open( FD, $SpacersFile )
      or die "Error in opening the input file : $SpacersFile!";
    my @lines = <FD>;
    close(FD);
    $i = 1;
...
"#=========================================================================\n";
    ...
    $File_Content .= "########################################\n";
    open WRITER, "> $Crispr_file"
      or die "The file $Crispr_file cannot be edited !\n";
    print WRITER $File_Content;
    close WRITER;
    return $Crispr_file;
}
...
sub Find_theCrispr {
    ...
    ( $simDRs, $refFalsSpacers, @spacers ) =
      definespacers( $DRlength, $crisprfile, $indexname );
    my @FalsSpacers = @$refFalsSpacers;
    if ( $#FalsSpacers >= 0 ) {
        for ( my $h = 0 ; $h <= $#FalsSpacers ; $h++ ) {
            $FalsSpacers[$h] += $seqbeg;
        }
    }
    ...
    return (
        $goodcrispr, $simDRs,   $CrisBeg, $CrisEnd,
        \%spacersH,  $#spacers, \@FalsSpacers
    );
}
...
sub check_short_crispr {
    ...
    # needle program
    $needleoptions =
"needle -auto -asequence DR -bsequence $spacerfile -gapopen 10.0 -gapextend 0.5 -outfile alignDR_Spacer.needle";
...
sub similarity_needle {
    ...
    open( FD, "alignDR_Spacer.needle" )
      or die "Error in opening the input file : alignDR_Spacer.needle!";
...
sub print_DR {
    ...
    if ( $#DR_array < 1 ) {
        ...
        open WRITER, "> $file_DRconsensus"
          or die "The file $file_DRconsensus cannot be edited !\n";
...
sub find_clusters    # find all possible clusters (even the shortest)
{
    ...
    $tabseq[$nb_clust] =
      $repetitions[$count]->Pos1;    # start position of the first cluster
    while ( $count < $#repetitions ) {
        if (
            !compare_clusters(
                $repetitions[$count]->Pos1,
                $repetitions[ $count + 1 ]->Pos1
            )
          )
        {
...
sub compare_clusters {
    ...
    if (   ( $element2 >= $element1 - 1500 )
        && ( $element2 <= $element1 + 1500 ) )
    {
        return 1;
    }
    else {
    ...
    for ( $argnum = 0 ; $argnum < $argcount - 1 ; $argnum++ ) {
            ...
            || $currentarg eq '-best' )
        {
            ...
            if ( $argnum >= $argcount - 1 || $argv[$argnum] =~ m/^\-/ ) {
                print STDERR
                  "$program: missing argument for option \"$currentarg\"\n";
                exit 1;
            }
...
{
    my @arg = @_;
    my ( @repetitions, @clusters_def )
      ;    #@clusters_def[0] is the index from which we begin clusters search,
    my $i
      ; #the other values of the array (when they exist) are resp. first_pos, length of hypothetical crisprs loci
    for ( $i = 1 ; $i <= $arg[0] + 1 ; $i++ ) {
        $repetitions[ $i - 1 ] = $arg[$i];
    }
    my $j = 0;
    ...
    if ( $test == 1 )    # a new cluster is found
    {
        ...
        while (
            (
                compare_clusters(
                    $repetitions[$ref]->Pos1,
                    $repetitions[ $count + 1 ]->Pos1
                )
            )
          )
        {
            if ( $repetitions[ $DR_min[0] ]->Length >
                $repetitions[ $count + 1 ]->Length )
            {
                @DR_min = ();
                $DR_min[0] = $count + 1;
            }
                ...
                $repetitions[ $count + 1 ]->Length )
            {
                if (
                    !(
                        $repetitions[ $DR_min[0] ]->DRseq eq
                        $repetitions[ $count + 1 ]->DRseq
                    )
                  )
                {
                    while ( $k <= $#DR_min ) {
                        if (
                            !(
                                $repetitions[ $DR_min[$k] ]->DRseq eq
                                $repetitions[ $count + 1 ]->DRseq
                            )
                          )
                        {
                            push( @DR_min, $count + 1 );
                        }    #add the new DR to DRs array
                        $k++;
                    }
                }
            }
        ...
        $pos_end = $repetitions[$count]->Pos1 +
          2 * $repetitions[ $DR_min[0] ]->Length + 1;
...
"\n ======There are $#DR_min+1 possible DRs of the same length in the sequence $file_DRconsensus =====\n\n";
            ...
            for ( $k = 0 ; $k <= $#DR_min ; $k++ ) {
                print WRITER
"> Hypothetical Direct repeat for the region around $pos_min\n";
                print WRITER $repetitions[ $DR_min[$k] ]->DRseq;
                print WRITER "\n";
            }
            close(WRITER);
            callmkvtree( $file_DRconsensus, $file_DRconsensus );
            my $taille = length( $repetitions[ $DR_min[0] ]->DRseq ) -
              2;    #case we have two differences
             # makesystemcall("./vmatch -l ".$taille . " -noevalue -noscore -supermax -noidentity -s leftseq ".$file_DRconsensus." > newtemp");
            makesystemcall( "vmatch -l "
                  . $taille
                  . " -noevalue -noscore -supermax -noidentity -s leftseq "
                  . $file_DRconsensus
                  . " > newtemp" );    #LK
            my @temp;
...
"> Hypothetical Direct repeat for the region around $pos_min\n";
            ...
            close(FD);
            unlink("newtemp")
              or die " can't delete the temporary file newtemp : !\n";
            callmkvtree( $file_DRconsensus, $file_DRconsensus );
        }
        else {
            print WRITER
...
sub printhelpbasic {
    my $programname = shift @_;
    print STDERR
      "Usage: $programname [options] <filename> <Results_directory>\n";
